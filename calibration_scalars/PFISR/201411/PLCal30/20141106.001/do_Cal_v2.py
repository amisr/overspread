...
exp = str(input('Enter YYYYMM : '))
exp = 'cal-%s' %(exp)
print(exp)
drive_num = str(input('Enter drive number (example: 004) : ')).zfill(3)
print(drive_num)
...
if __name__ == '__main__':
    ...
    if read_from_file == True:
        ...
        filepath = '/Volumes/PFISR_%s/Data AMISR Poker/'%(drive_num) + os.path.basename(os.getcwd()).split("-")[0] + '/'
    ...
    fnames = ['filelist_ac.txt','filelist_lp.txt']
    ...
    if read_from_file == True:
        ...
        # From raw file
        filelist = []
        filepath = '/Volumes/PFISR_004/Data AMISR Poker/' + os.path.basename(os.getcwd()).split("-")[0] + '/'
        dirList = os.listdir(filepath)
        for fname in dirList:
            if fname.endswith('.dt0.h5'):
                filelist.append(fname)
