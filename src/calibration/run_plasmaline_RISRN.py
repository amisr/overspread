...
#    INT = '_lp_1min'; 	LONGPULSE=1; doChap=0; usePower=0; Ilen=1; useAltPl=0
    INT='_ac_3min'; 	LONGPULSE=0; doChap=0; usePower=0; Ilen=3; useAltPl=0

#	ST=''; ILdir='/Volumes/AMISR_004/processed_data/PFISR/2010/PLCal30'
#	ST=''; ILdir='/Volumes/AMISR_004/processed_data/PFISR/2010/Erickson30/';
 #   ST='IPY17'+ST0 + '-'; ILdir='/Volumes/AMISR_004/processed_data/PFISR/2010/IPY17/';
#    ST=''; ILdir='/Volumes/ISR_DATA/processed_data/RISR/2010/WorldDay54m/';
    ST=''; ILdir='/Volumes/ISR_DATA_02/processed_data/RISR-N/2012/09/WorldDay64m/'
...
if __name__ == '__main__':
    ...
    ddir = '20120331.001'
    EXP = 'WorldDay64m'
    dddir = '/Volumes/ISR_DATA_02/processed_data/PFISR/2012/03/'
    ...
    ddir = '20120904.001'
    EXP = 'WorldDay64m'
    dddir = '/Volumes/ISR_DATA_02/processed_data/RISR-N/2012/09/'
    FILELIST_UP= os.path.join(dddir,EXP,ddir,'filelist_PL1.txt')
    FILELIST_DN = os.path.join(dddir,EXP,ddir,'filelist_PL2.txt')
    ODIR=os.path.join(dddir,EXP,ddir,'cal-0')
    DAT_PATH = '/Volumes/AMISR_016/Data AMISR Resolute N/' + ddir
    DIR_SEARCH = 1
    Is=200
    beams2do=[60617,63443,63587,64280] #beams 6,8,9,10
    MAXFILES=10
    flip=0 # down is really up.
    dualpl=1
    Ifiles=[]
    ...
    while not done:
        ...
        for jjj in range(Nbeams):
            ...
            if len(rrI)>=1:
                ...
                pylab.figure(figg1.number)
                pylab.subplot(211)
                pylab.imshow(pl_up,vmin=0,vmax=1e6,aspect='auto',origin='lower',extent=[freq_up[0]/1e6,freq_up[-1]/1e6,0,time_up[-1]-time_up[0]])
                pylab.colorbar()
                ax1=pylab.gca()
                ax1.set_ylabel(x_up)
                ax1.set_xlabel('Freq (MHz)')
                ...
                if dualpl:
                    pylab.subplot(212)
                    pylab.imshow(pl_dn,vmin=0,vmax=1e6,aspect='auto',origin='lower',extent=[freq_dn[0]/1e6,freq_dn[-1]/1e6,0.0,time_dn[-1]-time_dn[0]])
                    pylab.colorbar()
                    ax2=pylab.gca()
