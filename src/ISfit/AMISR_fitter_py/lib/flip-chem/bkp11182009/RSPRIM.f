...
C
 777  RETURN
      END
C::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
      SUBROUTINE SCOLUM(J,CHI,Z,TNJ,XN,YYYYDDD,UTSEC,GLATD,GLOND,LTHRS,
     >  F107A,F107,AP,COLUMN)
      IMPLICIT REAL(A-H,O-Z)
      INTEGER YYYYDDD
...
C++++ angle, re & ge radius and grav con for earth
     ...
     1    / 1.662E-24 ,   16. ,  32. ,  28. ,6.357E8, 980/
      ...
      IF(CHI.GE.1.5708) RETURN     !.. is sza>90.0 degrees
      ...
      IF(ALTG.GE.85*1.0E5) THEN
        ...
        SN(2)=DG(4)
        TNJ=T(2)
        !WRITE(6,'(2F10.2,1p,9E10.2)') ZG,TNJ,(SN(I),I=1,3)
        !STOP
      ELSE
        RETURN
      ENDIF
...
C...... Empirical Te model from Brace and Theis GRL p275, 1978
      ...
      REAL H,NE,TN,TB
      IF(NE.LT.1000) NE=1000
	IF(NE.GT.4E6)  NE=4E6
      TB=1051+(17.07*H-2746)*
     >  EXP(-5.122E-4*H+6.094E-6*NE-3.353E-8*H*NE)
      IF(TB.LT.TN) TB=TN
