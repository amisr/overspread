...
C... The EUVAC model is used for solar EUV irradiances
	...
	!.. various ionization and excitation rates by EUV and PE
      ...
      ALTCHEM=100  !.. Initial altitude for O+ for imposing chemistry
 ...
 5    CONTINUE 
      ...
      DO ITERS=1,2
        K=K+1          !.. Counts number of iterations
     ...
     >   ,OP2D,OP2P,HEPLUS,NPLUS,NNO,N4S)
	  ...
	  !.. Note that N(2D) is turned off in N+ solution 
        DISNP=EUVION(3,4)+EUVION(3,5)+EUVION(3,6)  !.. EUV N+ prod
        CALL CNPLS(JPRINT,10,K,ALT,RTS,OXN,O2N,N2N,NE,DISNP,NPLUS,
     >    OXPLUS,0.0,OP2P,HEPLUS,PHOTN,O2PLUS,N4S,OP2D,N2PLUS,NNO)
     ...
     >    ,N2D,N4S,N2P,NNO,O2PLUS,OXPLUS,OTHPR2(2),OTHPR2(1),N2A)
        
        NNO=PNO/LNO     !.. NO chemical equilibrium density
        !.. Set a floor on NO density, which is needed below ~150 km at night 
        IF(NNO.LT.8.0E7*EXP((100-ALT)/20)) 
     >     NNO=0.5*(NNO+8.0E7*EXP((100-ALT)/20))
     ...
     >    ,O2PLUS,TPROD5,OXPLUS,OP2D,N2PLUS,NPLUS,N4S,NNO,OP2P)
      ...
      IF(ALT.GT.0) OXPLUS=NE
 9    DO ITS=1,2
        IF(ITS.EQ.1) H=OXPLUS*0.01         !.. increment for dn/dt
     ...
     >     ,N2PLUS,DISN2D,UVDISN,NPLUS,N2P,N2D,OXPLUS,NNO,N2A)
      ...
      IF(JITER.GT.5.OR.OXPLUS.LT.0.0.OR.OXPLUS.GT.1.0E7) THEN
