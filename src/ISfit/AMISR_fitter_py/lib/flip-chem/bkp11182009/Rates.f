...
C.... 2001, page 21,305. Rates different from Fox and Sung indicated by PGR
 ...
 898  RTS(ITJS)=0.0
	...
	!.. Therefore the Fox and Sung parameterization is not appropriate
      ...
      !.. Zipf (1980) gives 2.2E-7*(300/TE)**0.39
      RTS(11)= 1.8E-7*(300/TE)**0.39
     ...
     >  -(0.456+0.174E-4*TE)*EXP(-2.97E4/TE))
      ...
      !.. N2 + O+(2P) -> N+ + NO  
      !.. Torr and Torr JGR, p 573 [1979] gave 1.0E-10 but Li et al.
      !..  J. Chem Phys. [1997] find it to be negligible  
      RTS(22)=0.0
      ...
      !.. N+ + O2 -> O2+ + N(4S) Dotan et al [1997]
      IF(TI.LE.1000) RTS(25)=2.02E-10*(300/TI)**-0.45
      IF(TI.GT.1000) RTS(25)=3.49E-10
      ...
      !.. O2 + N+ -> O(3P) + NO+    Dotan et al [1997]
      IF(TI.LE.1000) RTS(30)=4.32E-11*(300/TI)**-0.45
      IF(TI.GT.1000) RTS(30)=7.47E-11
      ...
      !.. N+ + O2 -> O2+ + N(2D)
      IF(TI.LE.1000) RTS(65)=8.65E-11*(300/TI)**-0.45
      IF(TI.GT.1000) RTS(65)=1.49E-10

      !.. N+ + O2 -> NO+ + O(1D)
      IF(TI.LE.1000) RTS(66)=1.75E-10*(300/TI)**-0.45
      IF(TI.GT.1000) RTS(66)=3.02E-10
...
C..  3   1.2E-12  300.0  2.0  0.45  0 0 0 0 
     ...
     >                     RTS)    !.. reaction rates
      ...
      REAL TE,TI,TN,RTS(99)  !.. See input parameters
