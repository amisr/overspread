...
C...... how to calculate electron heating rate and 3371 excitation rate.
      SUBROUTINE SECIPRD(ALT,SZADEG,F107,F107A,TE,TN,OXN,O2N
     >    ,N2N,XNE,SECPRD,N2APRD)
      INTEGER I,K,IK           !-- loop control variables
      INTEGER IDIM             !.. Array dimensions
      INTEGER EMAX             !.. Maximum PE energy
      PARAMETER (IDIM=501)
      REAL PEFLUX(IDIM)        !-- PE flux
      REAL UVFAC(59)           !-- Solar EUV multiplication factors
      REAL SIGIT(3)            !-- Total electron impact cross sections
      REAL SECPRD(3,6)         !-- Secondary production rates
      ...
      REAL EUV(9)              !-- PE mult. factors for bins 1-9 Torr et al.
      REAL SPRD(3,6)
     ...
     >             ,.00, 0.0,0.0,0.02/
      ...
      IF(F107.GT.60.0) THEN
        CALL FACEUV(UVFAC,F107,F107A)
         DO 10 I=1,9
           EUV(I)=UVFAC(I)  !.. EUV is used for PEs
 10      CONTINUE
      ENDIF
      ...
      !.. Go and get the photoelectron fluxes
       ...
       CALL FLXCAL(IDIM,ALT,SZADEG,TE,TN,EUV,XN,XNE,XN2D,XOP2D,PEFLUX,
     >    AFAC,EMAX)
      !***************************************************************
      ...
      DO I=1,EMAX
        E=I-0.5
        CALL TXSION(E,SIGIT)   !.. total ion cross sections
...
C------ AFAC = the solar EUV attenuation warning flag
      SUBROUTINE FLXCAL(IDIM,ALT,SZADEG,TE,TN,EUV,XN,XNE,XN2D,XOP2D
     >   ,PEFLUX,AFAC,EMAX)
...
C     > ,2.5,2,2.1,2.6,2.4,2,1.3,2.2,1.6,2,1,1.4,1.1,.5,4*.3,6*.2,33*.1 !.. old values
C     > ,100*0.05,100*0.04,100*0.01,100*0.02/ !.. old values


      SZA = SZADEG/57.29578   !.. convert solar zenith angle to radians

      EMIN=1      !.. minimum photoelectron energy
      EMAX=300    !.. maximum photoelectron energy. 300 is good
