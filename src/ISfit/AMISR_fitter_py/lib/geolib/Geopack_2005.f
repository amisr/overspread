...
C     by B. Rideout on Aug. 30, 2005.  A description of this
C     code can be found
...
c     5. dfloat changed to dble
C
C     $Id: Geopack_2005.f,v 1.4 2010/08/11 13:54:45 brideout Exp $
c
c     ##########################################################################
c     #                                                                        #
c     #                             GEOPACK-2005                               #
...
cwritten by N. A. Tsyganenko in 1978-1979. This version is dated May 04, 2005. On that
cdate, the IGRF coefficients were updated according to the recently published table of
cIGRF-10 coefficients, so that the main field model now extends through 2010 (a linear
cextrapolation is used for 2005 - 2010, based on the table of secular velocities). For
cmore details, see  http://www.ngdc.noaa.gov/IAGA/vmod/igrf.html (revision of 03/22/2005).
c
...
cbelow. Please consult the documentation file geopack-2005.doc (also available from this
c     site) for detailed descriptions of individual subroutines.
c
cThe following changes were made to the previous release of GEOPACK (of Jan 5, 2001).
c
...
cinvocation of TS_RECALC (otherwise they will be overridden by those returned by
c     TS_RECALC).
c
...
C
c
c      SUBROUTINE IGRF_GEO(R,THETA,PHI,BR,BTHETA,BPHI)
c
C     removed by Bill Rideout.  All routines needing igrf now call igrf11.f methods
C
c==========================================================================================
c
...
C     .. Common blocks ..
      ...
      DOUBLE PRECISION AAA(10),BBB(23),G(105),H(105),REC(105)
C     ..
...
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ATAN,ATAN2,COS,DMOD,SIN,SQRT,DBLE
C     ..
C     .. Data statements ..
...
C     ..
C     .. Local Arrays ..
      DOUBLE PRECISION DG05(45),DH05(45),G00(105),G05(105),G65(105),
     *                 G70(105),G75(105),G80(105),G85(105),G90(105),
     *                 G95(105),H00(105),H05(105),H65(105),H70(105),
     *                 H75(105),H80(105),H85(105),H90(105),H95(105)
C     ..
C     .. External Subroutines ..
      EXTERNAL TS_SUN
C     ..
...
C     .. Common blocks ..
     ...
     *                 SL0,SPS,ST0,STCL,STSL,XMUT
      DOUBLE PRECISION BA(6),G(105),H(105),REC(105)
C     ..
C     .. Data statements ..
c
     ...
     *     -0.5D0,-0.9D0/
      DATA G05/0.D0,-29556.8D0,-1671.8D0,-2340.5D0,3047.D0,1656.9D0,
     *     1335.7D0,-2305.3D0,1246.8D0,674.4D0,919.8D0,798.2D0,211.5D0,
     *     -379.5D0,100.2D0,-227.6D0,354.4D0,208.8D0,-136.6D0,-168.3D0,
     *     -14.1D0,72.9D0,69.6D0,76.6D0,-151.1D0,-15.0D0,14.7D0,-86.4D0,
     *     79.8D0,-74.4D0,-1.4D0,38.6D0,12.3D0,9.4D0,5.5D0,2.0D0,24.8D0,
     *     7.7D0,-11.4D0,-6.8D0,-18.0D0,10.0D0,9.4D0,-11.4D0,-5.0D0,
     *     5.6D0,9.8D0,3.6D0,-7.0D0,5.0D0,-10.8D0,-1.3D0,8.7D0,-6.7D0,
     *     -9.2D0,-2.2D0,-6.3D0,1.6D0,-2.5D0,-0.1D0,3.0D0,0.3D0,2.1D0,
     *     3.9D0,-0.1D0,-2.2D0,2.9D0,-1.6D0,-1.7D0,1.5D0,-0.2D0,0.2D0,
     *     -0.7D0,0.5D0,1.8D0,0.1D0,1.0D0,4.1D0,-2.2D0,-0.3D0,0.3D0,
     *     0.9D0,-0.4D0,1.0D0,-0.4D0,0.5D0,-0.3D0,-0.4D0,0.0D0,-0.4D0,
     *     0.0D0,-0.2D0,-0.9D0,0.3D0,0.3D0,-0.4D0,1.2D0,-0.4D0,0.7D0,
     *     -0.3D0,0.4D0,-0.1D0,0.4D0,-0.1D0,-0.3D0/
      DATA H05/0.D0,0.0D0,5080.0D0,0.0D0,-2594.9D0,-516.7D0,0.0D0,
     *     -200.4D0,269.3D0,-524.5D0,0.0D0,281.4D0,-225.8D0,145.7D0,
     *     -304.7D0,0.0D0,42.7D0,179.8D0,-123.0D0,-19.5D0,103.6D0,0.0D0,
     *     -20.2D0,54.7D0,63.7D0,-63.4D0,0.0D0,50.3D0,0.0D0,-61.4D0,
     *     -22.5D0,6.9D0,25.4D0,10.9D0,-26.4D0,-4.8D0,0.0D0,11.2D0,
     *     -21.0D0,9.7D0,-19.8D0,16.1D0,7.7D0,-12.8D0,-0.1D0,0.0D0,
     *     -20.1D0,12.9D0,12.7D0,-6.7D0,-8.1D0,8.1D0,2.9D0,-7.9D0,5.9D0,
     *     0.0D0,2.4D0,0.2D0,4.4D0,4.7D0,-6.5D0,-1.0D0,-3.4D0,-0.9D0,
     *     -2.3D0,-8.0D0,0.0D0,0.3D0,1.4D0,-0.7D0,-2.4D0,0.9D0,-0.6D0,
     *     -2.7D0,-1.0D0,-1.5D0,-2.0D0,-1.4D0,0.0D0,-0.5D0,0.3D0,2.3D0,
     *     -2.7D0,0.6D0,0.4D0,0.0D0,0.0D0,0.3D0,-0.8D0,-0.4D0,1.0D0,
     *     0.0D0,-0.7D0,0.3D0,1.7D0,-0.5D0,-1.0D0,0.0D0,0.7D0,0.2D0,
     *     0.6D0,0.4D0,-0.2D0,-0.5D0,-1.0D0/
      DATA DG05/0.0D0,8.8D0,10.8D0,-15.0D0,-6.9D0,-1.0D0,-0.3D0,-3.1D0,
     *     -0.9D0,-6.8D0,-2.5D0,2.8D0,-7.1D0,5.9D0,-3.2D0,-2.6D0,0.4D0,
     *     -3.0D0,-1.2D0,0.2D0,-0.6D0,-0.8D0,0.2D0,-0.2D0,2.1D0,-2.1D0,
     *     -0.4D0,1.3D0,-0.4D0,0.0D0,-0.2D0,1.1D0,0.6D0,0.4D0,-0.5D0,
     *     0.9D0,-0.2D0,0.2D0,-0.2D0,0.2D0,-0.2D0,0.2D0,0.5D0,-0.7D0,
     *     0.5D0/
      DATA DH05/0.0D0,0.0D0,-21.3D0,0.0D0,-23.3D0,-14.0D0,0.0D0,5.4D0,
     *     -6.5D0,-2.0D0,0.0D0,2.0D0,1.8D0,5.6D0,0.0D0,0.0D0,0.1D0,
     *     1.8D0,2.0D0,4.5D0,-1.0D0,0.0D0,-0.4D0,-1.9D0,-0.4D0,-0.4D0,
     *     -0.2D0,0.9D0,0.0D0,0.8D0,0.4D0,0.1D0,0.2D0,-0.9D0,-0.3D0,
     *     0.3D0,0.0D0,-0.2D0,0.2D0,0.2D0,0.4D0,0.2D0,-0.3D0,0.5D0,
     *     0.4D0/
C     ..
C
      IY = IYEAR
...
C
      DT = DBLE(IY) + DBLE(IDAY-1)/365.25D0 - 2005.D0
      DO 30 N = 1,105
         G(N) = G05(N)
         H(N) = H05(N)
         IF (N.GT.45) GO TO 30
...
C
 9000 FORMAT (/,/,1X,
     * '**** TS_RECALC WARNS: YEAR IS OUT OF INTERVAL 1965-2010: IYEAR='
