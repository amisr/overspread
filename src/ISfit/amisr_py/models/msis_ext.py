...
def compute_nuen(d,mn=[(0,4),(1,16),(2,28),(3,32),(6,1),(7,14)],Te=1000.0):
...
def compute_nuin(d,mi,mn=[(0,4),(1,16),(2,28),(3,32),(6,1),(7,14)],Ti=1000.0,Tn=1000.0):
...
def call_MSIS(ct_msis,doy,hrUT,glat,glong,year,altkm,ap,f107a,f107,z50,mass=[30]):
    ...
    qOp=compute_mfrac(altkm,Texo,t120[1],z50=z50[0],a=z50[1],Hinf=z50[2])
...
def iterate_MSIS(ct_msis,doy,hrUT,glat,glong,year,geophys_path,altkm=scipy.arange(50.0,1000.0,30.0),z50=185.0,mass=[30]):
    ...
    MSISout['nu_in']=scipy.zeros((altkm.shape[0]))   
...
def read_geophys(year,doy,curtime,geophys_dir):
    ...
    if os.path.exists(os.path.join(geophys_dir,str(year)))==False:
        raise IOError, 'Cannot read geophys param file %s.' % os.path.join(geophys_dir,str(year))
    ...
    if doy==1:
        try:
            F107D = float(lines_py[-1][65:71])
        except: # if can't get prev day, just get current day
            F107D = float(lines[doy-1][65:71])
    else:
        F107D = float(lines[doy-2][65:71])
    ...
    F107A= sum([float(l[65:71]) for l in lines2])/len(lines2)
...
def run_MSIS(ct_msis,yr,doy,hrUT,altkm,glat,glong,ap_array,f107a,f107,ap=-1,mass=[30]):
    ...
    for ia in range(len(mass)):
        nu_in[ia] = compute_nuin(output.d,mass[ia],Ti=output.t[1],Tn=output.t[1])
    nu_en = compute_nuen(output.d,Te=output.t[1])
