...
# compute_velvec2
def compute_velvec2(PLAT,AllVlos,AlldVlos,Allk,AllPlat,AllPlong,Allht,htmin=150.0*1000,htmax=400.0*1000,covar=[1000.*1000.,1000.*1000.,5.*5.],femax=0.5,emax=100.0):
	# 
	# This function takes the line of sight velocities and resolves them into vector velocities as a function of magnetic latitude.  
...
#	snru=20. # max SNR in db	
	...
	fracerrs=scipy.absolute(AlldVlos)/(scipy.absolute(AllVlos)+100.0)
...
#		I=scipy.where((AllPlat>plat_in[i])&(AllPlat<plat_in[i+1])&(Allht>htmin)&(Allht<htmax)&(tsnrdb>snrl)&(tsnrdb<snru)&(scipy.isfinite(tvlos)))[0]				
		I=scipy.where((AllPlat>plat_in[i,0])&(AllPlat<plat_in[i,1])&(Allht>htmin)&(Allht<htmax)&(scipy.isfinite(AllVlos))&(fracerrs<femax)&(abserrs<emax))[0]				
		Vest[i,:]=scipy.nan
		dVest[i,:]=scipy.nan
		if len(I) != 0:
			try:
