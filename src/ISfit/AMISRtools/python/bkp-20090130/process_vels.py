...
def dovels(fnameIn='',makeplot=1,plot2=0,zoom=[],saveout=1,plats=[[66.0,69.0,0.25,0.25],[]],clim=[-1000,1000],sc=[15.0,15.0],Time2Integrate=[600.0],swaphr=0,MinAlt=175.0,MaxAlt=450.0,PPP=[200.0,0.5,2000.0,100.0],COVAR=[3000.*3000.,3000.*3000.,15.*15.],CorrectVap=1,zoomWhole=[]):

#	fname="/Volumes/AMISR_004/processed_data/2008/IPY05/20080404.004/IPY05-20080404.004_lp_15min-cal.h5"
#	fname="/Volumes/AMISR_004/processed_data/2007/IPY02/20070901.002/IPY02-20070901.002_lp_15min-cal.h5"
#	fname="/Volumes/AMISR_004/processed_data/Sporadic03/20080215.005/20080215.005_lp_5min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Mathews01/20080408.002/20080408.002_lp_2min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Thayer01/20070906.001/20070906.001_lp_1min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Hysell01/20080117.001/20080117.001_lp_1min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Joule2/20070119.001/20070119.001_lp_1min-launch.h5"
#	fname="/Volumes/AMISR_004/processed_data/2008/WorldDay03/20080215.001/20080215.001_lp_2min-cal.h5"
	fname="/Volumes/AMISR_004/processed_data/2008/Lyons02/20080213.001.done/20080213.001_lp_3min-cal.h5"
#	fname="/Volumes/AMISR_004/processed_data/2008/Erickson12/20080929.001/20080929.001_lp_2min-cal.h5"
#	fname="/Volumes/AMISR_004/processed_data/2007/LTCS01/20071209.006/20071209.006_lp_2min-cal.h5"
#	fname="/Volumes/AMISR_004/processed_data/BjornG01/20080312.001/20080312.001_lp_2min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Watkins02/20080308.001/20080308.001_lp_2min.h5"
#	fname="/Volumes/AMISR_004/processed_data/Student/Student03/20080709.003/20080709.003_lp_1min.h5"
	...
	ofname='%s-vvels-%dsec' % (os.path.basename(fname)[:-3],Time2Integrate[0])
	ofnamehr='%s-vvels-%dsec' % (os.path.basename(fname)[:-3],Time2Integrate[-1])
	...
	if CorrectVap:
		IupB=scipy.where(BeamCodes[:,0]==64157.0)[0]
		InotUpB=scipy.where(BeamCodes[:,0]!=64157.0)[0]
		print IupB
...
#	MLTtime1=scipy.zeros(timeout.shape,dtype=timeout.dtype)
#	MLTtime1[:,0]=getmlt(timeout[:,0],plong[0,0])
#	MLTtime1[:,1]=getmlt(timeout[:,1],plong[0,0])
	...
	if plot2==1 and len(Time2Integrate)==1:
		...
		while not done:
			...
			MLTtimehr2.append([MLT[Irecs[0],0],MLT[Irecs[-1],1]])
			if IIrec>0:
				if MLTtimehr2[IIrec][0]<MLTtimehr2[IIrec-1][0]:
					MLTtimehr2[IIrec][0]=MLTtimehr2[IIrec][0]+24.0
					MLTtimehr2[IIrec][1]=MLTtimehr2[IIrec][1]+24.0

			if IIrec==0:
				vvelshr2=Vest[scipy.newaxis,:,:]
				dvvelshr2=dVest[scipy.newaxis,:,:]
				Nall2=Nmeas[scipy.newaxis,:]
			else:
				...
				Nall2=scipy.concatenate((Nall2,Nmeas[scipy.newaxis,:]),axis=0)
		...
		MLTtimehr2=scipy.array(MLTtimehr2)

		"""
		MLTtimehr2=scipy.zeros(timeouthr2.shape,dtype=timeouthr2.dtype)
		MLTtimehr2[:,0]=getmlt(timeouthr2[:,0],plong[0,0])
		MLTtimehr2[:,1]=getmlt(timeouthr2[:,1],plong[0,0])		
		"""
	...
	if saveout==1:
		...
		# hr
		if plot2==1:
			print 'Writing output to' + os.path.join(odir,ofnamehr+'.h5')
			outh5file=tables.openFile(os.path.join(odir,ofnamehr+'.h5'), mode = "a", title = "Fit File")			
			write_outputfile(outh5file,timeouthr,groupname='Time',name='UnixTime')
			write_outputfile(outh5file,dtimeouthr,groupname='Time',name='dtime')
			write_outputfile(outh5file,MLTtimehr,groupname='Time',name='MagneticLocalTime')
			write_outputfile(outh5file,Nall2.astype('int32'),groupname='VectorVels',name='Nmeas')
			write_outputfile(outh5file,vvelshr,groupname='VectorVels',name='Vest')
			write_outputfile(outh5file,dvvelshr,groupname='VectorVels',name='dVest')
			write_outputfile(outh5file,PLAT_OUT,groupname='VectorVels',name='Plat')
			write_outputfile(outh5file,Paramhr,keys2do=Paramhr.keys(),groupname='ProcessingParams')
			outh5file.close()
		
	if makeplot==1:	
		figg=plot_vvels1(timeout,scipy.mean(MLTtime1,axis=1),PLAT_OUT,vvels1[:,:,1],vvels1[:,:,0],dvvels1[:,:,1],dvvels1[:,:,0],
			title=title,p=PPP,sc=sc[0],cax=clim,ncols=3,vz=vvels1[:,:,2],dvz=dvvels1[:,:,2],vzsc=10.0)
		if saveout:
			figg.savefig(os.path.join(odir,ofname+'.png'))
		if plot2:
			if swaphr==0:
				TI=Time2Integrate[0]
				txt1='  %d sec, top' % (TI)
				...
				figg=plot_vvels(timeout,timeouthr,timeout,scipy.mean(MLTtime1,axis=1),scipy.mean(MLTtimehr,axis=1),scipy.mean(MLTtime1,axis=1),PLAT_OUT,PLAT_OUT,
					vvels1[:,:,1],vvels1[:,:,0],dvvels1[:,:,1],dvvels1[:,:,0],
					vvelshr[:,:,1],vvelshr[:,:,0],dvvelshr[:,:,1],dvvelshr[:,:,0],
					vvels1[:,:,1],vvels1[:,:,0],dvvels1[:,:,1],dvvels1[:,:,0],
					title=title,p=PPP,sc=sc,txt1=txt1,txt2=txt2,cax=clim,
					ncols=3,vz=vvels1[:,:,2],dvz=dvvels1[:,:,2],vz=vvelshr[:,:,2],dvz=dvvelshr[:,:,2],vzsc=10.0)
				...
				figg=plot_vvels(timeout,timeouthr,timeouthr2,scipy.mean(MLTtime1,axis=1),scipy.mean(MLTtimehr,axis=1),scipy.mean(MLTtimehr2,axis=1),PLAT_OUT,PLAT_OUT,
					vvels1[:,:,1],vvels1[:,:,0],dvvels1[:,:,1],dvvels1[:,:,0],
					vvelshr[:,:,1],vvelshr[:,:,0],dvvelshr[:,:,1],dvvelshr[:,:,0],
					vvelshr2[:,:,1],vvelshr2[:,:,0],dvvelshr2[:,:,1],dvvelshr2[:,:,0],
					title=title,p=PPP,sc=sc,txt1=txt1,txt2=txt2,cax=clim,
					ncols=3,vz=vvels1[:,:,2],dvz=dvvels1[:,:,2],vzhr=vvelshr2[:,:,2],dvzhr=dvvelshr2[:,:,2],vzsc=10.0)
			
			if saveout:
				figg.savefig(os.path.join(odir,ofnamehr+'.png'))

def dovelsAltcomb(makeplot=1,plot2=0,zoom=[],saveout=1,alts=[[80.0,200.0,5.0],[200.0,400.0,50.0]],clim=[-1000,1000],sc=[15.0,15.0],Time2Integrate=[600.0],swaphr=0,MinAlt=80.0,MaxAlt=450.0,MinEl=0.0,MaxEl=90.0,MinAz=-180.0,MaxAz=180.0):

#	fname='/Users/mnicolls/Documents/Work/ISfit/AMISR_fitter_py/runfiles/jouleII/20070301.013/20070301.013_ac_5min.h5'
	fname1='/Volumes/AMISR_004/processed_data/joule2/20070117.004/20070117.004_ac_10min-phaseerrs-noteti.h5'
	fname2='/Volumes/AMISR_004/processed_data/joule2/20070117.004/20070117.004_lp_1min.h5'
...
def dovelsAlt(makeplot=1,plot2=0,zoom=[],saveout=1,alts=[80.0,250.0,5.0],clim=[-1000,1000],sc=[15.0,15.0],Time2Integrate=[600.0],swaphr=0,MinAlt=175.0,MaxAlt=450.0,MinEl=0.0,MaxEl=90.0,MinAz=-180.0,MaxAz=180.0,COVAR=[3000.*3000.,3000.*3000.,3000.*3000.],PPP=[200.0,0.5,2000.0,100.0]):
...
def dovelsAltGeo(makeplot=1,plot2=0,zoom=[],saveout=1,alts=[80.0,250.0,5.0],clim=[-1000,1000],sc=[15.0,15.0],Time2Integrate=[600.0],swaphr=0,MinAlt=175.0,MaxAlt=450.0,MinEl=0.0,MaxEl=90.0,MinAz=-180.0,MaxAz=180.0,COVAR=[3000.*3000.,3000.*3000.,3000.*3000.],PPP=[200.0,0.5,2000.0,100.0]):
...
def plot_vvels(time,timehr,timecol,MLTtime,MLTtimehr,MLTtimecol,lat,lat2,vx,vy,dvx,dvy,vxhr,vyhr,dvxhr,dvyhr,vxcol,vycol,dvxcol,dvycol,title='Vector Vels',p=[200.0,0.25,4000.0,500.0],sc=[15.0,10.0],txt1='',txt2='',cax=[-1000,1000],label='Mag. Lat. (degrees)',ncols=2,vz=[],dvz=[],vzhr=[],dvzhr=[],vzsc=1.0,nrows=4,geo=0):
	...
	ax[ii].set_title('V perp east (m/s)', fontsize=labsize, horizontalalignment='center')
...
def plot_vvels1(time,MLTtime,lat,vx,vy,dvx,dvy,title='Vector Vels',p=[200.0,0.25,4000.0,500.0],sc=15.0,cax=[-1000,1000],label='Mag. Lat. (degrees)',ncols=2,vz=[],dvz=[],vzsc=1.0,nrows=4,geo=0):
	...
	ax[ii].set_ylabel(label, fontsize=labsize)
	if geo==1:
		ax[ii].set_title('V east (m/s)', fontsize=labsize, horizontalalignment='center')
	else:
		ax[ii].set_title('V perp east (m/s)', fontsize=labsize, horizontalalignment='center')
	ax[ii].text(xlim[0],(lat.max()-lat.min())*0.15+lat.max(),title,fontsize=labsize, horizontalalignment='left')
	...
	if ncols==3:
		...
		pylab.setp(ax[ii], yticklabels=[])
		if geo==1:
			ax[ii].set_title('V up (m/s) x %d' % (vzsc), fontsize=labsize, horizontalalignment='center')	
		else:
			ax[ii].set_title('V anti par (m/s) x %d' % (vzsc), fontsize=labsize, horizontalalignment='center')	
	...
	if ncols==3:
		...
		pylab.setp(ax[ii], yticklabels=[])
		if geo==1:
			ax[ii].set_title('err V up (m/s) x %d' % (vzsc), fontsize=labsize, horizontalalignment='center')					
		else:
			ax[ii].set_title('err V anti par (m/s) x %d' % (vzsc), fontsize=labsize, horizontalalignment='center')
	...
	Q=ax[ii].quiver(X,Y,scipy.transpose(vx),scipy.transpose(vy),C,scale=1000*sc,width=rect[2]*0.005)
	ax[ii].quiverkey(Q, 1.1, 1, cax[1], str(cax[1]) + ' m/s',fontproperties={'size' : labsize},labelpos='S')
	ax[ii].xaxis.tick_bottom()
	ax[ii].set_xlabel('Time (UT)', fontsize=labsize)
	labels = pylab.getp(ax[ii], 'xticklabels')
	pylab.setp(labels, fontsize=textsize)
