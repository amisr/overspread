...
DATAPATH='/data/ISR_Data/processed_data/2008'
...
if __name__ == '__main__':
	...
	DISK_PATH='/Volumes/AMISR_004/processed_data/2008' # volume where data is
	EXP_TYPE='Erickson12'
	EXP_NAME_WC='200809*.*' # experiment wild card
	OverwriteExisting = 1	
...
#	Plots2do1 = []; PlotsTitles1 = []
	...
	dirs=glob.glob(os.path.join(FILE_PATH,EXP_NAME_WC)); dirsdone=glob.glob(os.path.join(FILE_PATH,'*.done')); dirs=list(set(dirs)-set(dirsdone))
	Ndirs=len(dirs)
	dirs.sort()
	...
	for aaa in range(Ndirs):
		...
		if (not os.path.exists(madname)) or OverwriteExisting:
			...
			files = list(set(glob.glob(os.path.join(dirs[aaa],'plots_*'))) - set(glob.glob(os.path.join(dirs[aaa],'*Lag*'))))
			Nfiles=len(files)
			Nfiles2=0
