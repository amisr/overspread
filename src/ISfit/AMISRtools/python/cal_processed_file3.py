#! /usr/bin/env python

"""
xxxxx

~M. Nicolls
last revised: xx/xx/2007

"""

import sys
import os.path
import tables
import scipy
import scipy.signal
import scipy.interpolate
import scipy.stats
import scipy.io
import glob
import shutil
import matplotlib
matplotlib.use('Agg')
import pylab
import datetime

sys.path.append('/Users/mnicolls/Documents/Work/ISfit/AMISR_fitter_py/src')

import plot_utils
import io_utils

##############################

def readafile(fname):

	h5file=tables.openFile(fname)
	output={}
	for group in h5file.walkGroups("/"):
		output[group._v_pathname]={}
		for array in h5file.listNodes(group, classname = 'Array'):						
			output[group._v_pathname][array.name]=array.read()		
	h5file.close()
	
	return output

"""
def write_outputfile(fhandle,dict2do,keys2do=[],groupname='',name=''):

	if groupname == '':
		group=fhandle.root
	else:
		if fhandle.__contains__('/'+groupname):
			group='/'+groupname
		else:
			group=fhandle.createGroup(fhandle.root, groupname, 'Dataset')

	if len(keys2do)==0:
		try:
			fhandle.removeNode(group,name)
		except:
			''
		fhandle.createArray(group,name, dict2do, "Dataset")
	else:
		for key in keys2do:
			fhandle.createArray(group, key, dict2do[key], "Dataset")
"""

if __name__ == '__main__':

		replot=0;

		corrChirp=1; chirpCorr=-20.0

		"""
		# Erickson
		odir='/Volumes/AMISR_004/processed_data/2009/Erickson12'; exp_WC='2008*.*[0-9]'; 
		tpAll=['lp_2min','ac_5min']
		calFname='/Users/mnicolls/Documents/Work/AMISR/calibration/Erickson02/Erickson12-calibration-04.01.2009.txt'
		st=''
		subDir='cal-0'
		useScaler=1 # scaler from plasma line
		useCalData=1 # beam-dependent factor
		"""

		"""
		# IPY
		odir='/Volumes/AMISR_004/processed_data/2008/IPY15'; exp_WC='20080902.002'; 
		calFname='/Users/mnicolls/Documents/Work/AMISR/ipy/cal_20080801.002-ipy15/IPY15-calibration-LP-12.08.2008.txt'
#		tpAll=['ac_15min-cal','ac_15min-64157-cal']
#		tpAll=['ac_15min-cal','lp_15min-cal']
		tpAll=['ac_15min-64157-cal', 'ac_15min-cal', 'lp_15min-64157-cal', 'lp_15min-cal']
		st='IPY15-'
		useScaler=0 # scaler from plasma line
		useCalData=1 # beam-dependent factor

		# IPY
		odir='/Volumes/AMISR_004/processed_data/2008/IPY05'; exp_WC='20080502.001'; 
		tpAll=['ac_15min-64157-cal-cal', 'lp_15min-64157-cal-cal','lp_15min-cal-cal','ac_15min-cal-cal']
		st='IPY05-'
		useScaler=0 # scaler from plasma line
		useCalData=0 # beam-dependent factor

		# Mis
		odir='/Volumes/AMISR_004/processed_data/2007/Joule2'; exp='20070119.001'; 
		#calFname='/Users/mnicolls/Documents/Work/AMISR/calibration/20071209.006/LTCS01-calibration-LP-10.10.2008.txt'		
		tpAll=['lp_2min', 'lp_5min', 'ac_5min-phaseerrs', 'ac_15min-phaseerrs']
		st=''
		useScaler=0 # scaler from plasma line
		useCalData=0 # beam-dependent factor
		"""

		# Lyons
		odir='/Volumes/AMISR_004/processed_data/2010/Lyons30'; exp_WC='200905*.*[0-9]'; 
		#tpAll=['lp_2min-cal','ac_5min-cal']
		tpAll=['lp_1min', 'lp_3min', 'ac_2min-rng2', 'ac_6min']
		#tpAll=['lp_1min', 'ac_2min-rng2']
		#tpAll=['lp_30sec']
		st=''
		useScaler=0 # scaler from plasma line
		useCalData=0 # beam-dependent factor
	
		"""
		# Sporadic
		odir='/Volumes/AMISR_004/processed_data/2007/Sporadic03'; exp_WC='2007*.*[0-9]';
		tpAll=['lp_5min-cal', 'bc_2min-Ne-cal']
		calFname='/Users/mnicolls/Documents/Work/AMISR/calibration/Sporadic03/Sporadic03-OctNovDec07--calibration-12.08.2008.txt'
		st=''
		subDir='cal-0'
		useScaler=0 # scaler from plasma line
		useCalData=1 # beam-dependent factor	
		"""
	
		# Bjorn
		odir = '/Volumes/AMISR_004/processed_data/2009/BjornG11'; exp_WC='20090308.00*'
		tpAll=['lp_1min']
		st=''
		useScaler=0 # scaler from plasma line
		useCalData=0 # beam-dependent factor	
	
		dirs=glob.glob(os.path.join(odir,exp_WC))
		Ndirs=len(dirs)
		dirs.sort()
	
		for expp in dirs:
			exp=os.path.basename(expp)
	
			if useCalData:
				calData=scipy.io.read_array(calFname)
			
			for ii in range(len(tpAll)):
				tp=tpAll[ii]
					
				datName=os.path.join(odir,exp,st+exp +'_' + tp +'.h5'); oName=os.path.join(odir,exp,st+exp + '_' + tp+ '-cal' + '.h5');  plotdir=os.path.join(odir,exp,'plots_' + tp)
				FitFile=1
				if datName.__contains__('-Ne'):
					FitFile=0

				# read datfile
				if os.path.exists(datName):
					h5dat=readafile(datName)
					mUnixTime=scipy.mean(h5dat['/Time']['UnixTime'],1)
					if FitFile:
						NeFit=h5dat['/FittedParams']['Ne']; (Nrecs,Nbeams,Nhts)=NeFit.shape
						dNeFit=h5dat['/FittedParams']['dNe']
						Fits=h5dat['/FittedParams']['Fits']
					Ne_NoTr=h5dat['/NeFromPower']['Ne_NoTr']; (x1,x2,Nhts2)=Ne_NoTr.shape
					Ne_Mod=h5dat['/NeFromPower']['Ne_Mod']
					BeamCodes=h5dat['/']['BeamCodes']
					Nbeams=Ne_Mod.shape[1]
				
					# scaler (plasma line)
					scaler=scipy.ones((Nbeams,2),dtype='float32')
					if useScaler:
						if os.path.exists(os.path.join(odir,exp,subDir,'CalFile-'+tp+'.txt')):
							scaler=scipy.io.read_array(os.path.join(odir,exp,subDir,'CalFile-'+tp+'.txt'))
							print 'Using scaler'
						else:
							print 'Not using scaler'
							xxxxx
					
					# beam-dependent
					if useCalData:
						tCalData=calData.copy()
						tCalData[:,4]=tCalData[:,4]*scaler[:,1]
				
						if FitFile:
							corrNeFit=NeFit.copy(); corrdNeFit=dNeFit.copy();
						corrNe_NoTr=Ne_NoTr.copy(); corrNe_Mod=Ne_Mod.copy();
						for ibm in range(Ne_Mod.shape[1]):
							ibm2do=scipy.where(tCalData[:,0]==BeamCodes[ibm,0])[0]
							if FitFile:
								corrNeFit[:,ibm,:]=corrNeFit[:,ibm,:]/tCalData[ibm2do,4]
								corrdNeFit[:,ibm,:]=corrdNeFit[:,ibm,:]/tCalData[ibm2do,4]
							corrNe_NoTr[:,ibm,:]=corrNe_NoTr[:,ibm,:]/tCalData[ibm2do,4]
							corrNe_Mod[:,ibm,:]=corrNe_Mod[:,ibm,:]/tCalData[ibm2do,4]	
					
						#
						if FitFile:
							h5dat['/FittedParams']['Ne']=corrNeFit
							h5dat['/FittedParams']['dNe']=corrdNeFit
						h5dat['/NeFromPower']['Ne_NoTr']=corrNe_NoTr
						h5dat['/NeFromPower']['Ne_Mod']=corrNe_Mod
					
					# chirp correction
					if corrChirp and FitFile:
						corrFits=Fits.copy()
						corrFits[:,:,:,:,-1]=corrFits[:,:,:,:,-1]+chirpCorr
						h5dat['/FittedParams']['Fits']=corrFits
				
					t=datetime.date.today()
					datestr=t.strftime("%Y-%m-%d")			
				
					# write datfile
					print 'Writing output to' + oName
					outh5file=tables.openFile(oName, mode = "w", title = "Fit File")
					for key in h5dat.keys():
						tgroup=str.split(key,'/')[-1]
						tloc=str.split(key,'/')[-2]
						if not tloc.startswith('/'):
							tloc='/'+tloc
						io_utils.write_outputfile(outh5file,h5dat[key],h5dat[key].keys(),groupname=tgroup,grouploc=tloc)
					if useCalData:
						io_utils.write_outputfile(outh5file,calData,groupname='Calibration',name='CalDataBeam')
						io_utils.write_outputfile(outh5file,datestr,groupname='Calibration',name='CalDateBeam')
					if useScaler:
						io_utils.write_outputfile(outh5file,scaler,groupname='Calibration',name='AdditionalScaler')
						io_utils.write_outputfile(outh5file,os.path.basename(calFname),groupname='Calibration',name='CalFileBeam')
					if corrChirp and FitFile:
						io_utils.write_outputfile(outh5file,chirpCorr,groupname='Calibration',name='ChirpCorrection')
					outh5file.close()
				
					if replot:
						plot_utils.replot_pcolor_all(oName,saveplots=1,opath=plotdir,clims=[[10,12],[0,1500],[0,3000],[0,4],[-500,500]],ylim=[],tlim=[])
			
