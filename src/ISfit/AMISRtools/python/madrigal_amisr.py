...
def parseExpId(expId):
    ...
    #if num < 0 or num > 26: Edited by Steven Chen
    #if num < 0 or num > 26:
...
class hdf5VelocityToMadrigal:
    ...
    def __init__(self,
        ...
        try:
            ...
            dvestArray = None         
            xxxxxx               
        dvestArray=scipy.maximum(dvestArray,minVErr)
            ...
            for platIndex in range(numPlat):
                ...
                for parmIndex in range(len(params)):
                    ...
                    # vi6 or vi3
                        ...
                        #print '%s-%f\n' % (params[parmIndex],vestArray[recIndex][platIndex][parallel_index]);
                        try:
                    ...
                    # dvi6 or dvi3
                        ...
                        #print '%s-%f\n' % (params[parmIndex],dvestArray[recIndex][platIndex][parallel_index]);
                        try:
                            if isNan(vestArray[recIndex][platIndex][parallel_index]) \
                                or isNan(dvestArray[recIndex][platIndex][parallel_index]) \
                                or scipy.absolute(dvestArray[recIndex][platIndex][parallel_index])<minVErr \
                    ...
                    # eu
                        ...
                        try:
                            if isNan(eestArray[recIndex][platIndex][parallel_index]) \
                                or isNan(deestArray[recIndex][platIndex][parallel_index]) \
                                or scipy.absolute(deestArray[recIndex][platIndex][parallel_index])<minEErr \
                                or scipy.absolute(deestArray[recIndex][platIndex][parallel_index])>maxEErr \
                                or scipy.absolute(eestArray[recIndex][platIndex][parallel_index])>maxE: 
                                raise ValueError, ''
