...
proc cgi_html_comment	{args}	{
    ...
    cgi_puts "<!--[_cgi_list_to_string $args] -->"
}
	...
	if 1==[catch $_cgi(body) errMsg] {
		...
		cgi_body {
			...
			if {$_cgi(debug) == "-on"} {
			    ...
			    cgi_mail_add "cgi.tcl version: 1.0.0"
			    cgi_mail_add "input:"
...
proc cgi_division {args} {
    cgi_put "<div"
    _cgi_close_proc_push "cgi_puts </div>"
...
proc cgi_preformatted {args} {
    cgi_put "<pre"
    _cgi_close_proc_push "cgi_puts </pre>"
...
proc cgi_number_list {args} {
    cgi_put "<ol"
    _cgi_close_proc_push "cgi_puts </ol>"
...
proc cgi_bullet_list {args} {
    cgi_put "<ul"
    _cgi_close_proc_push "cgi_puts </ul>"
...
proc cgi_definition_list {cmd} {
    cgi_puts "<dl>"
    _cgi_close_proc_push "cgi_puts </dl>"
...
proc cgi_menu_list {cmd} {
    cgi_puts "<menu>"
    _cgi_close_proc_push "cgi_puts </menu>"
...
proc cgi_directory_list {cmd} {
    cgi_puts "<dir>"
    _cgi_close_proc_push "cgi_puts </dir>"
...
proc cgi_unbreakable {cmd} {
    cgi_puts "<nobr>"
    _cgi_close_proc_push "cgi_puts </nobr>"
    uplevel 1 $cmd
    _cgi_close_proc
}
...
proc _cgi_input_multipart {fin} {
	...
	if {[info exists filename]} {
	    ...
	    _cgi_set_uservar $varname [list $foutname $filename]
...
proc cgi_map {name cmd} {
    ...
    _cgi_close_proc_push "cgi_puts </map>"
...
proc cgi_file_button {name args} {
    ...
    cgi_puts "<input type=file name=\"$name\"[_cgi_list_to_string $args]>"
}
...
proc cgi_select {name args} {
    ...
    _cgi_close_proc_push "cgi_puts </select>"
    foreach a [lrange $args 0 [expr [llength $args]-2]] {
...
proc cgi_table {args} {
    cgi_put "<table"
    _cgi_close_proc_push "cgi_puts </table>"
...
proc cgi_caption {args} {
    cgi_put "<caption"
    _cgi_close_proc_push "cgi_puts </caption>"
...
proc cgi_table_row {args} {
    cgi_put "<tr"
    _cgi_close_proc_push "cgi_puts </tr>"
    if {[llength $args]} {
	cgi_put "[_cgi_lrange $args 0 [expr [llength $args]-2]]"
    }
    cgi_puts ">"
...
proc cgi_table_head {args} {
    cgi_put "<th"
    _cgi_close_proc_push "cgi_puts </th>"
...
proc cgi_table_data {args} {
    cgi_put "<td"
    _cgi_close_proc_push "cgi_puts </td>"
...
if {[info tclversion] >= 7.5} {
	...
	#proc $_cgi(new) {args} "upvar 1 _cgi_local x; set x \$args; uplevel 1 \"$_cgi(old) \$x\""
    }
}
