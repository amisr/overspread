C     $Id: conduct.f,v 1.2 2006/10/26 20:44:36 brideout Exp $
...
CC  NOTE: O+/O INCREASED BY BURNSIDE FACTOR = 1.3
        TR = (TI + TN)/2.D0 
        NUI(1) = 1.3*(3.67D-11*N(1)*SQRT(TR) * (1-0.064*LOG10(TR))**2)
     *         + 6.82D-10*N(2)
     *         + 6.64D-10*N(3)
...
CC O2+ VS O, N2 (NONRESONANT) AND O2 (RESONANT)
        NUI(3) = 2.31D-10*N(1)
     *         + 4.13D-10*N(2)
     *         + 2.59D-11*N(3)*SQRT(TR) * (1-0.073*LOG10(TR))**2
CC ELECTRONS VS O, N2 AND O2
        NUE    = 8.9D-11*N(1)*(1+5.7D-4*TE)*SQRT(TE)
     *         + 2.33D-11*N(2)*(1-1.21D-4*TE)*TE
     *         + 1.82D-10*N(3)*(1+3.6D-2*SQRT(TE))*SQRT(TE)
CC
